...
class Gui(tk.Tk):
    ...
    #Socket Block - Telemetry for Usage
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    host = '??' # TODO Server not available atm.
    port = '??'# TODO Server not available atm.
    try:
        s.connect((host,port))
        serverOnline = True
        intialHandshake = "CLIENT: " + str(datetime.utcnow()) +  ":" + username + ":" + "Connected using [" + versionTag + "]"
    ...
    #EXAMPLE SEND MESSAGE TO TELEMETRY SERVER.
    unpackMetrics = "SomeString."
    try:
    ...
    def render_initial_config(self):
        ...
        # Second, render UI based on values above.
        self.MasterPane.add(self.Workbench, sticky='nsew') # Always rendered
...
class Tk_EditCaseMenu(tk.Toplevel):
    ...
    def save_changes(self, event=None):
        ...
        print("would UPDATED", self.key_value, "RECORD WITH...")
        pprint.pprint(new_sr_dict)
        ...
        for col_dict in target_case.template.value:
            ...
            for workpane in col_dict["workpanes"]:
                ...
                if workpane[0] == widget:
                    ...
                    # Update template.value - which calls target Frames "update_panes"
...
# Creating Config.JSON and "basecamp.db"
bcamp_setup.CreateConfig()
